...
describe("sortReturns", () => {
  ...
  it("không biến đổi mảng gốc", () => {
    const original = [...TEST_RETURNS];
    sortReturns(TEST_RETURNS, "TotalAmount", "asc");
    expect(TEST_RETURNS).toEqual(original);
  });
});

describe("calcStats", () => {
  it("đếm đúng done / pending", () => {
    const stats = calcStats(TEST_RETURNS);
    const done    = TEST_RETURNS.filter((r) => r.Status === "Hoàn thành").length;
    const pending = TEST_RETURNS.filter((r) => r.Status === "Đang xử lý").length;
    expect(stats.done).toBe(done);
    expect(stats.pending).toBe(pending);
    expect(stats.total).toBe(TEST_RETURNS.length);
  });

  it("totalAmount chỉ tính Hoàn thành", () => {
    const stats = calcStats(TEST_RETURNS);
    const expected = TEST_RETURNS
