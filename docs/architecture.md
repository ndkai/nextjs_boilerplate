...
Ứng dụng là frontend cho nghiệp vụ nhập trả hàng SCA, chạy trên Next.js App Router với React 19 và TypeScript strict. Báo cáo này mô tả kiến trúc chung của toàn project, không chỉ riêng một chức năng.

Các route chính hiện có:

- `app/inputsale`: tạo phiếu nhập trả.
- `app/returns`: tra cứu lịch sử nhập trả.
- `app/page.tsx`: trang điều hướng/tổng quan.
...
- [app/returns/page.tsx](/Users/habuiduc/Downloads/nextjs_boilerplate-main/app/returns/page.tsx:1)
...
- [app/returns/_hooks/use-returns-list.ts](/Users/habuiduc/Downloads/nextjs_boilerplate-main/app/returns/_hooks/use-returns-list.ts:1)
...
Ví dụ feature phức tạp trong project:
...
Ví dụ feature đơn giản hơn:

- [app/returns/_lib/api.ts](/Users/habuiduc/Downloads/nextjs_boilerplate-main/app/returns/_lib/api.ts:1)
- [app/returns/_lib/repository.ts](/Users/habuiduc/Downloads/nextjs_boilerplate-main/app/returns/_lib/repository.ts:1)
- [app/returns/_lib/types.ts](/Users/habuiduc/Downloads/nextjs_boilerplate-main/app/returns/_lib/types.ts:1)
...
### 7.2 Ví dụ flow nghiệp vụ nhiều bước
...
Flow này là ví dụ cho một nghiệp vụ phức tạp. Các feature đơn giản hơn có thể đi ngắn hơn:
...
Không bắt buộc mọi feature đều phải có `domain/`, `mappers/` hoặc `use-cases/`. Chỉ thêm các folder đó khi feature có logic thật sự cần tách.
...
### 9.1 Khi nào move code lên `lib/`

Không move code lên `lib/` chỉ vì muốn “nhìn chung chung hơn”. `lib/` chỉ nên chứa code thật sự dùng chung hoặc infrastructure toàn app.

| Loại code | Đặt ở feature | Move lên `lib/` |
| --- | --- | --- |
| UI component riêng màn hình | Có | Không |
| Hook orchestration riêng màn hình | Có | Không |
| Use case riêng nghiệp vụ màn hình | Có | Không |
| API/repository chỉ một feature dùng | Có | Chưa cần |
| API/repository nhiều feature dùng chung | Không nên nhân bản | Có |
| Format/helper dùng nhiều feature | Không | Có |
| Auth, DI, apiClient | Không | Có |

Nếu nhiều feature cùng dùng chung backend resource, có thể tách data-access theo hướng:
...
Khi đó `app/<feature>/_lib/use-cases/` vẫn giữ flow nghiệp vụ riêng của màn hình, còn `lib/sca/*` chỉ giữ data-access dùng chung.
